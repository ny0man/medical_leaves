...
class Ref_menu extends MY_Controller {
	
	var $nama_menu = 'Ref Menu';
	var $nama_controller = 'Ref_menu';
	var $nama_tabel = 'ref_menu';
	var $primary_tabel = 'refMenuId';
	...
	public function validasi_field(){
		...
		$this->form_validation->set_rules('refMenuNama', 'Nama', 'required');
	...
	public function simpan(){
		...
		$db_result = 0;
		$parameter = array(
			'refMenuNama' => $this->input->post('refMenuNama'),
			'refMenuInfo' => $this->input->post('refMenuInfo'),
			'refMenuUrutan' => $this->input->post('refMenuUrutan'),
			'refMenuIcon' => $this->input->post('refMenuIcon') 
		);

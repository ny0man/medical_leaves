							...
							<?php foreach($data as $key => $value): ?>
								<tr>
									<td>
										...
										onclick="confirm_delete('<?= $value['outBenNama'] ?>','<?= site_url($this->nama_controller.'/delete')?>','<?= $value[$this->primary_tabel] ?>')"></button>
									</td>                    
									<td><?= $value['outBenNama'] ?></td>
									<td><?= number_format($value['outBenNominal'],0,",",".") ?></td>
									<td><?= date("d M Y", strtotime($value['outBenStart'])) ?></td>
									<td><?= date("d M Y", strtotime($value['outBenEnd'])) ?></td> 
								</tr>
							<?php endforeach ?>
							</tbody>
						</table>
